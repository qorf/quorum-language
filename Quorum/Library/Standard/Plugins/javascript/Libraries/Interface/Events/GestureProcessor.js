...
function plugins_quorum_Libraries_Interface_Events_GestureProcessor_() {
    let gestureListener = null;
    this.gestureEvents = undefined;
    this.InitializeGestureProcessor$quorum_text = function(containerID) {
            // setup gesture listeners
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', function(){
                    InitializeHammer(containerID)
                });
            } else {
                this.InitializeHammer(containerID);
            } 
    ...
    this.InitializeHammer = function(containerID) {
        ...
        gestureListener.on("panstart panmove panend pancancel", function(ev) {
            if(ev.pointerType === "touch") {
                ...
                if(ev.type === "panstart") {
                    gestureEvent.timingCode = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__BEGIN_();
                } else if(ev.type === "panmove") {
                    gestureEvent.timingCode = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__CONTINUE_();
        ...
        gestureListener.on("swipe twoFingerSwipe threeFingerSwipe", function(ev) {
            if(ev.pointerType === "touch") {
                ...
                if(ev.offsetDirection === Hammer.DIRECTION_LEFT) {
                    gestureEvent.direction = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__LEFT_();
                }
                if(ev.offsetDirection === Hammer.DIRECTION_RIGHT) {
                    gestureEvent.direction = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__RIGHT_();
                }
                if(ev.offsetDirection === Hammer.DIRECTION_UP) {
                    gestureEvent.direction = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__UP_();
                }
                if(ev.offsetDirection === Hammer.DIRECTION_DOWN) {
                    gestureEvent.direction = gestureEvent.Get_Libraries_Interface_Events_GestureEvent__DOWN_();
                }
                let gameInfo = plugins_quorum_Libraries_Game_WebInput_.GetGestureGameInfo(ev);
                if (gameInfo !== null)
                {
