...
//        }
    ...
    public void visitBlock(MethodExecution method, OpcodeTracker tracker) {
        ...
        stack.pushScope(new ScopeLabel(startLabel, endLabel));
        ...
        //visit the this
        visitMeLocalVariable(currentClassExecution);
        
        //visit all local variables
    ...
    public void visit(AlwaysEndStep step) {
        ...
        BlockDescriptor blockAtLine = step.getMethodDescriptor().getBlockAtLine(step.getBeginLine());
        if (blockAtLine != null) {
    ...
    public void visit(BeginDetectScopeStep step) {
        ...
        step.getDetectParameters().get(0).getDetectParameter().setIsCaughtException(true);
    ...
    public void visit(BeginScopeStep step) {
        if (step.getBlockTag().equals("always")) {
            ...
            Label startLabel = new Label();
            methodVisitor.visitLabel(startLabel);
            stack.pushScope(new ScopeLabel(startLabel, new Label()));
        }
    ...
    public void visit(CallStep step) {
            ...
            if (step instanceof ParentCallStep) {
                ...
                if (field) {//if this is a call on a field object
                    ...
                    if (var instanceof ParameterDescriptor) {
                        ...
                        //stack.
                        int number = -1;
                            if(var.isCaughtException()){
                                number = var.getVariableNumber();
                            }else{
                                number = var.getVariableNumber() - currentClass.getNumberOfVariables();
                            }
                        int mapped = stack.getMappedVariableNumber(number, var.getType(), false);
                        methodVisitor.visitVarInsn(QuorumConverter.getLoadOpcode(var.getType()), mapped);
                    }
        ...
        if (!isParameter && !isCalledOnInterface && isStaticCallOnPrimitive) {
    ...
    public void visit(EndScopeStep step) {
        if (!stack.isEmptyLabel()) {//if the stack has a label
            ...
            if (label.getLabelType().equals(LabelTypeEnum.IF) && !step.getBlockTag().equals("always")) {//if the label is for an if statement
                ...
                //visit any 'always' code that needs to be inserted.
                CheckDetectDescriptor desc = stack.peekCheckDetect();
                ...
                if (label.getLabelType().equals(LabelTypeEnum.CHECK)) {
                    ...
                    stack.peekScope().endLabel = desc.getCheckEnd();
                    BlockDescriptor blockAtLine = step.getMethodDescriptor().getBlockAtLine(step.getBeginLine());
                    if(blockAtLine != null){
                        visitLocalVariable(blockAtLine);
                        stack.popScope();
                    ...
                    if (label1 != null && endLabel == null) {
                        ...
                        methodVisitor.visitLabel(endLabel);
                        stack.peekScope().endLabel = endLabel;
                        BlockDescriptor blockAtLine = step.getMethodDescriptor().getBlockAtLine(step.getBeginLine());
                        if(blockAtLine != null){
                            visitLocalVariable(blockAtLine);
                            stack.popScope();
                ...
                boolean found = false;
                while (desc.getAlwaysStartPosition() != -1 && !found) {
                    ExecutionStep futureStep = steps.get(alwaysEndPosition);
                    if (futureStep instanceof AlwaysEndStep) {
                        AlwaysEndStep endStep = (AlwaysEndStep) futureStep;
                        if (desc.getBlockName().equals(endStep.getCheckDetectLabel())) {
                            found = true;
                        } else {
                            alwaysEndPosition++;
                        }
        ...
        if(LOCAL_VARIABLE_NAMES_VISITED){
            ...
            while(scopeVariables.hasNext()){
                ...
                if(next.getVariableNumber() != -1){
                    ...
                    ScopeLabel popScope = stack.peekScope();
                    methodVisitor.visitLocalVariable(next.getName(), QuorumConverter.convertTypeToBytecodeString(next.getType()), QuorumConverter.convertTypeToBytecodeString(next.getType()), popScope.startLabel, popScope.endLabel, stack.getParameterNumber(next.getStaticKey()));
                }
            }
        }
    }
    ...
    private void visitMeLocalVariable(ClassExecution currentClassExecution) {
        if(LOCAL_VARIABLE_NAMES_VISITED){
