...
<script type="text/javascript">
    ...
    slideArray[4] = "<h5>Calling Actions</h5><ul><li>If we run our code now, we should notice that the code in our <code>OutputTest</code> action never executes.  This is because the program always starts at the <code>Main</code> action and we never told it to run the code in our other action.</li><li>The last thing we need to do now is call the <code>OutputTest</code> action from our <code>Main</code> action.</li><li>We can accomplish this by adding the line <code>OutputTest()</code> in our Main action.  The parentheses are necessary at the end of the action name for the computer to know it's an action.</li><span class=\"title\">Try it!</span><div class =\"task\"><p>Add a call to OutputTest within the Main action with the code <code>OutputTest()</code> and try running it.</p></div></ul>";
    
    slideArray[5] = "<h5>Actions: More Information</h5><ul><li>We've covered the very basics of actions up to this point.</li><li>Actions for the online version of Quorum are currently limited, but the offline version can do things such as pass parameters and return values.</li><li>For more information on actions, click <a href=\"http://quorumlanguage.com/documents/syntax/actions.php\">here</a></li></ul>";
