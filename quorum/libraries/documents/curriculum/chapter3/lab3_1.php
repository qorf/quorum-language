...
<p>
Enter a value of 1 in the input dialog.  Nothing should have happened.  Add the code to tell Sodbeans to speak the price right after the line starting with <tt>integer price =</tt>. Run the program again. This time, notice that the number "20,000" is only spoken if we enter the number 1.
</p>
<p>
One problem with this solution is the user only knows the price of their order if they order a single car. To correct this, move the say statement we wrote to after the line starting with <tt>end</tt>. This puts the code outside of the if statement, so it will always execute. Run the program. Whoops!  We are getting the following compiler errors:
</p>
<p><pre class="code"><code>
...
<p>
The problem is, we defined the <tt>price</tt> variable inside of the if statement. In Quorum, this means it only exists inside the <i>scope</i> of the if statement, between the <tt>if</tt> and <tt>end</tt> keywords. This is what is called <i>lexical scoping</i>, and it is very useful to keep a program organized. In this case, however, we want the <tt>price</tt> variable to be accessible outside of the if statement's scope. To fix this, define the <tt>price</tt> variable above the if statement, and remove the <tt>integer</tt> keyword from the line inside the if statement. The compiler error we witnessed before should no longer be present. Before we run the program, add a statement to print the value of the <tt>price</tt> variable after the line containing the <tt>end</tt> keyword. When we run the program, the input dialog will appear as before.
</p>
<p>
Our program behaves properly when we enter a value of one. However, we have not yet considered what happens when we enter a value greater than one. Run the program and enter a larger value. Note the program's behavior. Since we did not account for this condition, the price is set to $0. When users want to purchase multiple cars, we need to multiply the price of $20,000 by the number of cars purchased. This is done by first using an else statement, as shown below. The else-statement should be executed only if the program skips the first if-statement, which occurs when <tt>numberOfCars</tt> does not equal 1. Our else statement looks like so:
</p>
<p><pre class="code"><code>
else
...
<p>
Run the program again, and note that the else statement is executed when we enter a value greater than one. Experiment by modifying the constant value 20000 inside the <tt>else</tt> block so that customers get a discount when purchasing multiple cars. What happens when we wish to purchase two cars? Justify to yourself that this behavior is different than if we had a constant price for all cases.
</p>
<h2>Task 2: More Work with Conditionals</h2>
<p>
What we have written so far isn’t enough for the car dealership to meet its business requirements.  We will need to make changes to our current code to comply with the business rules.  Use the business rules below and translate the descriptions in plain English to the code.
</p>
...
<p>
First, we need to implement additional checking on the user’s numerical input.  Run the program and enter any negative value in the input dialog. Notice that the price we give the user is a negative value. This would imply that the car dealership is giving away money! The application should not allow this to occur. In order to fix this problem, write an if statement immediately before the <tt>if</tt> block we created earlier. This if statement should determine if <tt>numberOfCars</tt> is less than one. If <tt>numberOfCars</tt> is less than one, set <tt>numberOfCars</tt> to 1.
</p>
<p>
