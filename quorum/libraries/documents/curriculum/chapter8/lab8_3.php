...
<p>
    As you can see from the above, the letters are directly exchanged for letter that corresponds to it in the Ciphertext alphabet. "h" at index 7 in the Plaintext alphabet corresponds to "s" at index 7 in the Ciphertext alphabet. To do all this, you will be using the <tt>Text</tt> class in Quorum, which has some powerful string manipulation actions needed for creating this cipher.
</p>
<p>
    Create a new project and name it <b>Lab8_3</b>. Create a second class and name it <tt>Cipher</tt>.
</p>
...
<p>
    Both actions will take a text value (the message to be encoded or decoded) from the user, encode or decode it, and then return the modified message. Start by creating three Text objects. One of them will hold the alphabet, one will hold the ciphertext alphabet, and the last will hold encoded message. You can use the <tt>SetValue</tt> action to set the values of the objects to the needed text. For example:
</p>
<pre class="code">
    Text myString
