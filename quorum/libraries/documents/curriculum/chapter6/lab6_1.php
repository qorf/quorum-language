...
<p>
You will create two additional classes in this project. In a New File dialog, create a new file by selecting “Quorum” and “Quorum Class” in the Categories and File Types windows, respectively. You can also access the New File dialog with the keyboard shortcut Ctrl + N. Then, name the new file <tt>Circle</tt> in a New Quorum Class dialog. Repeat the above steps to create a Class named <tt>Sphere.</tt>
</p>
<p>
  You will specify code for <tt>Circle</tt>, <tt>Sphere</tt>, and <tt>Main</tt> classes in the next three tasks.
</p>
...
<p>
  You'll now create actions that will calculate various circle and sphere properties. The sphere calculations will require a radius, which can only be obtained through inheritance from class <tt>Circle</tt>
</p>
<p>
  <li>
...
<p>
<tt>CalculateVolume</tt> requires a <tt>number</tt> type parameter and returns a number. As the name indicates, this action will calculate the volume of a sphere given its radius. The equation for sphere volume is: (4/3)*pi*radius cubed.
</p>
<p>
  <li>
...
<p>
  <tt>CalculateSurfaceArea</tt> requires a <tt>number</tt> type parameter and returns a number. This action calculates the surface area of a sphere given its radius. The equation for sphere surface area is: 4*pi*radius squared.
</p>
<p>
  Now it's time to use these actions. In <tt>Main</tt>, call the actions you created above, using the radius you obtained from the <tt>Sphere</tt> class as the arguments. Create a say statement that tells the user what the radius is that was used, and what the calculated number is.
</p>
<p>
...
<p>
  <tt>CalculateArea</tt> uses a number type parameter and returns a number. This action will calculate the area of a circle given its radius. The equation for circle area is: pi*radius squared.
</p>
<p>
  <li>
    <b>
...
<p>
  <tt>CalculateCircumference</tt> uses a number type parameter and returns a number. This action will calculate the circumference of a circle given its radius. The equation for circumference is 2*pi*radius
</p>
<p>
  Now, just as you did with the sphere actions, call the actions you created above in <tt>Main</tt>, using the radius obtained from the <tt>Circle</tt> actions. Create a say statement that tells the user what the radius is that was used, and what the calculated number is.
</p>
<h2>
  Sample Output
</h2>
<p>
  Run the program and notice that a different radius is used for the circle and sphere calculations. This is because you are able to set different values using the same method. With inheritance, you were able to use the get and set actions created in class <tt>Circle</tt> to create a unique radius for the sphere calculations. At the same time, you could still use those same actions to create a unique radius for the circle calculations. In the case of class <tt>Sphere</tt>, inheritance allowed you to reuse code that had already been created. When run, your program should look similar to this:
</p>
<p>
  <pre class="code">
    <code>
