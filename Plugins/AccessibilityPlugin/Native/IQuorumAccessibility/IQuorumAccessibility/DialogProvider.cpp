...
DialogProvider::DialogProvider(_In_ DialogControl* control) : m_refCount(1), control(control)
{
}
...
IFACEMETHODIMP_(ULONG) DialogProvider::AddRef()
{
	return InterlockedIncrement(&m_refCount);
}

IFACEMETHODIMP_(ULONG) DialogProvider::Release()
{
...
IFACEMETHODIMP DialogProvider::QueryInterface(_In_ REFIID riid, _Outptr_ void** ppInterface)
{
...
IFACEMETHODIMP DialogProvider::get_ProviderOptions(_Out_ ProviderOptions* pRetVal)
{
...
IFACEMETHODIMP DialogProvider::GetPatternProvider(PATTERNID patternId, _Outptr_result_maybenull_ IUnknown** pRetVal)
{
	if (!IsWindow(control->GetHWND()))
	{
		return UIA_E_ELEMENTNOTAVAILABLE;
...
IFACEMETHODIMP DialogProvider::GetPropertyValue(PROPERTYID propertyId, _Out_ VARIANT* pRetVal)
{
	if (!IsWindow(control->GetHWND()))
	{
		return UIA_E_ELEMENTNOTAVAILABLE;
	...
	{
		pRetVal->vt = VT_BSTR;
		pRetVal->bstrVal = SysAllocString(L"dialog");
	}
...
IFACEMETHODIMP DialogProvider::get_HostRawElementProvider(_Outptr_result_maybenull_ IRawElementProviderSimple** pRetVal)
{
