...
public class Array<T> implements Iterable<T> {
	...
	public T selectRanked (Comparator<T> comparator, int kthLowest) {
		if (kthLowest < 1) {
			throw new GameRuntimeError("nth_lowest must be greater than 0, 1 = first, 2 = second...");
		}
...
// System.out.println(iterable.lastAcquire);
				throw new GameRuntimeError("#iterator() cannot be used nested.");
			}
...
// System.out.println(iterable.lastAcquire);
		...
		public void remove () {
			if (!allowRemove) throw new GameRuntimeError("Remove not allowed.");
			index--;
