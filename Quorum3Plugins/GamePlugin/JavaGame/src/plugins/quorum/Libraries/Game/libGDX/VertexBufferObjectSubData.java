...
public class VertexBufferObjectSubData implements VertexData {
	...
	private int createBufferObject () {
		...
		GameState.nativeGraphics.glBindBuffer(GraphicsManager.GL_ARRAY_BUFFER, result);
		GameState.nativeGraphics.glBufferData(GraphicsManager.GL_ARRAY_BUFFER, byteBuffer.capacity(), null, usage);
		GameState.nativeGraphics.glBindBuffer(GraphicsManager.GL_ARRAY_BUFFER, 0);
		return result;
	}
	...
	private void bufferChanged () {
		if (isBound) {
			GameState.nativeGraphics.glBufferSubData(GraphicsManager.GL_ARRAY_BUFFER, 0, byteBuffer.limit(), byteBuffer);
			isDirty = false;
		}
	}
	...
	public void updateVertices (int targetOffset, float[] vertices, int sourceOffset, int count) {
		...
		if (isDirect) {
			...
			throw new GameRuntimeError("Buffer must be allocated direct."); // Should never happen
