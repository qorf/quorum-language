...
public class Mesh implements Disposable {
	...
	/** list of all meshes **/
	static final Map<quorum.Libraries.Game.Application_, Array<Mesh>> meshes = new HashMap<quorum.Libraries.Game.Application_, Array<Mesh>>();
	...
	protected Mesh (VertexData vertices, IndexData indices, boolean isVertexArray) {
		...
		addManagedMesh(GameState.GetApp(), this);
	}
	...
	public void dispose () {
		if (meshes.get(GameState.GetApp()) != null) meshes.get(GameState.GetApp()).removeValue(this, true);
		vertices.dispose();
		indices.dispose();
	}
	...
	public void calculateBoundingBox (BoundingBox bbox) {
		...
		if (numVertices == 0) throw new GameRuntimeError("No vertices defined");
	...
	public BoundingBox extendBoundingBox (final BoundingBox out, int offset, int count, final Matrix4 transform) {
		...
		if (offset < 0 || count < 1 || offset + count > numIndices)
			throw new GameRuntimeError("Not enough indices ( offset=" + offset + ", count=" + count + ", max=" + numIndices
				+ " )");
	...
	public float calculateRadiusSquared (final float centerX, final float centerY, final float centerZ, int offset, int count,
		...
		int numIndices = getNumIndices();
		if (offset < 0 || count < 1 || offset + count > numIndices) throw new GameRuntimeError("Not enough indices");
	...
	private static void addManagedMesh (quorum.Libraries.Game.Application_ app, Mesh mesh) {
	 ...
	 * @param app */
	public static void invalidateAllMeshes (quorum.Libraries.Game.Application_ app) {
	...
	public static void clearAllMeshes (quorum.Libraries.Game.Application_ app) {
		meshes.remove(app);
	}
	...
	public static String getManagedStatus () {
		...
		for (quorum.Libraries.Game.Application_ app : meshes.keySet()) {
